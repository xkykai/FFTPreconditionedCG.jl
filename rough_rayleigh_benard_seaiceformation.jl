...
filename = "rough_RB_seaiceformation_Ra_$(Ra)_Pr_$(Pr)_Nr_$(Nr)_Lx_$(Lx)_Lz_$(Lz)_Nx_$(Nx)_Nz_$(Nz)_$(pressure_solver_str)"
...
free_slip_bc = FluxBoundaryCondition(0)

u_bcs = FieldBoundaryConditions(top=no_slip_bc, bottom=free_slip_bc, immersed=no_slip_bc)
v_bcs = FieldBoundaryConditions(top=no_slip_bc, bottom=free_slip_bc, immersed=no_slip_bc)
