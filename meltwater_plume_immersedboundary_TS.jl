...
using Printf
using JLD2
using Oceananigans.Grids: xnodes, znodes, with_number_type
...
filename = "meltwater_plume_immersedgrid_TS_$(advection_str)_Ra_$(Ra)_Pr_$(Pr)_Lx_$(Lx)_Lz_$(Lz)_Nx_$(Nx)_Nz_$(Nz)"
...
const τ = 10 / sqrt(N²)
@inline T_timeramp(i, j, k, grid, clock, model_fields, p) = p.B / p.α / p.g * tanh(clock.time / p.τ)
...
const l = (4 * ν * κ / N²)^(1/2)
Tbar = Average(T, dims=(2, 3))
Sbar = Average(S, dims=(2, 3))

simulation.output_writers[:jld2] = JLD2Writer(model, (; u, w, T, S, c, d, pNHS = model.pressures.pNHS);
