...
pressure_solver = ConjugateGradientPoissonSolver(immersed_grid, maxiter=100; preconditioner)
...
function initial_conditions!(model)
    ...
    set!(model, T = initial_T, S = initial_S, c = initial_c)
end
...
function setup_simulation(model)
    ...
    outputs = (; u, v, w, T, S, c, b, d, p)
    averaged_outputs = (; Tbar, Sbar, cbar)

    OUTPUT_PATH = "./Data/$(prefix)_mwe_fields_constantS_noslip"
...
#%%
T_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "T")
S_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "S")
c_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "c")
b_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "b")
d_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "d")
p_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_fields.jld2", "p")

T_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "T")
S_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "S")
c_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "c")
b_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "b")
d_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "d")
p_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_fields.jld2", "p")
#%%
Tbar_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_averaged.jld2", "Tbar")
Sbar_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_averaged.jld2", "Sbar")
cbar_data_fft = FieldTimeSeries("./Data/gaussian_ridge_fft_mwe_fields_constantS_noslip_averaged.jld2", "cbar")

Tbar_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_averaged.jld2", "Tbar")
Sbar_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_averaged.jld2", "Sbar")
cbar_data_cg = FieldTimeSeries("./Data/gaussian_ridge_cg_mwe_fields_constantS_noslip_averaged.jld2", "cbar")
#%%
...
axc_fft = Axis(fig[1, 3], xlabel = "x (m)", ylabel = "z (m)", title = "Passive tracer (FFT solver)")
axd_fft = Axis(fig[1, 4], xlabel = "x (m)", ylabel = "z (m)", title = "Divergence (FFT solver)")
axp_fft = Axis(fig[1, 5], xlabel = "x (m)", ylabel = "z (m)", title = "Pressure (FFT solver)")
axb_fft = Axis(fig[1, 6], xlabel = "x (m)", ylabel = "z (m)", title = "Buoyancy (FFT solver)")
...
axc_cg = Axis(fig[3, 3], xlabel = "x (m)", ylabel = "z (m)", title = "Passive tracer (FFT-preconditioned CG solver)")
axd_cg = Axis(fig[3, 4], xlabel = "x (m)", ylabel = "z (m)", title = "Divergence (FFT-preconditioned CG solver)")
axp_cg = Axis(fig[3, 5], xlabel = "x (m)", ylabel = "z (m)", title = "Pressure (FFT-preconditioned CG solver)")
axb_cg = Axis(fig[3, 6], xlabel = "x (m)", ylabel = "z (m)", title = "Buoyancy (FFT-preconditioned CG solver)")

axTbar = Axis(fig[5, 1:2], xlabel = "time (days)", ylabel = "T (°C)", title = "Domain-averaged temperature")
axSbar = Axis(fig[5, 3:4], xlabel = "time (days)", ylabel = "S (psu)", title = "Domain-averaged salinity")
axcbar = Axis(fig[5, 5:6], xlabel = "time (days)", ylabel = "c", title = "Domain-averaged concentration")

n = Observable(Nt)

T_fftₙ = @lift interior(T_data_fft[$n], :, 1, :)
S_fftₙ = @lift interior(S_data_fft[$n], :, 1, :)
c_fftₙ = @lift interior(c_data_fft[$n], :, 1, :)
...
c_cgₙ = @lift interior(c_data_cg[$n], :, 1, :)
...
clim_FFT = (find_min(interior(c_data_fft[timeframe_lim])[interior(c_data_fft[timeframe_lim]) .!= 0]), find_max(interior(c_data_fft[timeframe_lim])[interior(c_data_fft[timeframe_lim]) .!= 0]))
...
clim_cg = (find_min(interior(c_data_cg[timeframe_lim])[interior(c_data_cg[timeframe_lim]) .!= 0]) - 1e-4, find_max(interior(c_data_cg[timeframe_lim])[interior(c_data_cg[timeframe_lim]) .!= 0]) + 1e-4)
...
cbarlim = (find_min(interior(cbar_data_fft)) - 1e-4, find_max(interior(cbar_data_fft)) + 1e-4)

colormap_symmetric = :balance
colormap_asymmetric = :plasma
...
hmc_FFT = heatmap!(axc_fft, xC, zC, c_fftₙ, colorrange = clim_FFT, colormap = colormap_asymmetric)
...
hmc_cg = heatmap!(axc_cg, xC, zC, c_cgₙ, colorrange = clim_cg, colormap = colormap_asymmetric)
...
Colorbar(fig[2, 3], hmc_FFT, label = "c", vertical=false, flipaxis=false)
Colorbar(fig[2, 4], hmd_FFT, label = "s⁻¹", vertical=false, flipaxis=false)
Colorbar(fig[2, 5], hmp_FFT, label = "Pa", vertical=false, flipaxis=false)
Colorbar(fig[2, 6], hmb_FFT, label = "m²/s²", vertical=false, flipaxis=false)
...
Colorbar(fig[4, 3], hmc_cg, label = "c", vertical=false, flipaxis=false)
Colorbar(fig[4, 4], hmd_cg, label = "s⁻¹", vertical=false, flipaxis=false)
Colorbar(fig[4, 5], hmp_cg, label = "Pa", vertical=false, flipaxis=false)
Colorbar(fig[4, 6], hmb_cg, label = "m²/s²", vertical=false, flipaxis=false)

lines!(axTbar, times ./ 1days, Tbar_cg, label="PCG")
lines!(axSbar, times ./ 1days, Sbar_cg, label="PCG")
lines!(axcbar, times ./ 1days, cbar_cg, label="PCG")

lines!(axTbar, times ./ 1days, Tbar_fft, label="FFT")
lines!(axSbar, times ./ 1days, Sbar_fft, label="FFT")
lines!(axcbar, times ./ 1days, cbar_fft, label="FFT")

vlines!(axTbar, timeₙ, color = :black)
vlines!(axSbar, timeₙ, color = :black)
vlines!(axcbar, timeₙ, color = :black)

ylims!(axTbar, Tbarlim)
ylims!(axSbar, Sbarlim)
ylims!(axcbar, cbarlim)
...
CairoMakie.record(fig, "./Output/cg_fft_gaussian_ridge_mwe_fields_constantS_noslip_Centered.mp4", 1:Nt, framerate=10, px_per_unit=2) do nn
    n[] = nn
end
#%%
